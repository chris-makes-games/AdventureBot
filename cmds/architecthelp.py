...
async def architecthelp(ctx, topic=None):
  ...
  "- `key1 = (key1 + key2)`\n(true if amount of key1 is equal to the sum of the amounts of key1 and key2. if key2 is anything but zero, it will be false.)",
  ...
  #for expressions information
    ...
    embed.add_field(name="Operators", value="Operators are symbols used to decide if an expression is true or false. The nine operators are: `and`, `or`, `not`, `=`, `!=`, `>`, `<`, `>=`, `<=`\n When operators are used with keys, they form an expression. You can optionally separate expressions with commas, or group them together with the below methods:", inline=False)
    ...
    embed.add_field(name="Missing Keys", value="If a player does not have one of the keys in the expression, that key will be treated as a zero. You can test for this with things like `key1 = 0 and not key2 = 0` which will return true if the player doesn't have either key. You can also add keys together that the player doesn't have, like `key1 + key2 > 3` would not add anything together unless the player has a key. If they have three of one of those keys, it would return true.", inline=False)
    ...
    embed.add_field(name="Examples", value=f"These are examples of valid expressions you can input into your rooms:\n{conditions_examples}", inline=False)
