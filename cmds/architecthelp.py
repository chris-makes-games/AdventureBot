...
async def architecthelp(ctx, topic=None):
  ...
  #for expressions information
    embed = discord.Embed(title="Expressions", description="Expressions are logical statements which can either be true or false. An expression being evaluated is known as returning. Expressions are exclusively used in the four Room fields: lock, unlock, hide, and reveal. If the expression is true, then one of those four things happens to the room.")
    embed.add_field(name="Operators", value="Operators are symbols used to decide if an expression is true or false. The operators are: `and`, `or`, `=`, `!=`, `<`, `>`, `>=`, `<=`\n When operators are used with keys, they form an expression. Separate expressions with commas.")
